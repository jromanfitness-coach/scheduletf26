...
export default async function handler(request) {
  // Uses Netlify's modern Request/Response Functions API. Netlify supplies Blobs context automatically.
  ...
  let store;
  try {
    store = getStore(STORE_NAME, { consistency: 'strong' });
  } catch (error) {
    console.error('scheduler-api storage initialization failed', error);
    return json(503, {
      ok: false,
      error: 'Shared scheduler storage is unavailable. Redeploy once, then retry.'
    });
  }
  ...
  try {
    ...
    return json(500, { ok: false, error: 'The scheduler server could not complete that request. Refresh once and retry. If it persists, check the Netlify scheduler-api function log.' });
  }
};
