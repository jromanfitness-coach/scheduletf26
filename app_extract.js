...
(() => {
  ...
  function managementMarkersHtml(s){const chips=[];if(s?.managementNewMember)chips.push('<span class="management-marker-chip">New Member</span>');if(s?.managementNewClient)chips.push('<span class="management-marker-chip">New Client</span>');return chips.join('')}
  ...
  function sessionDraft(existing,seed){return existing?clone(existing):{id:'',date:seed.date,time:seed.time,coachId:seed.coachId,classTypeId:seed.classTypeId||state.classTypes[0]?.id,kind:'Group',capacity:10,durationMinutes:60,status:'active',notes:'',managementNewMember:false,managementNewClient:false,repeatWeeks:0,repeatDays:[],createdAt:new Date().toISOString()}}
  function openSession(opts={}){
    ...
    function capture(){const g=id=>document.getElementById(id);if(!g('sType'))return;d={...d,classTypeId:g('sType').value,coachId:g('sCoach').value,date:g('sDate').value,time:g('sTime').value,capacity:+g('sCap').value,status:g('sStatus').value,notes:g('sNotes').value,managementNewMember:g('mgmtNewMember')?g('mgmtNewMember').classList.contains('active'):!!d.managementNewMember,managementNewClient:g('mgmtNewClient')?g('mgmtNewClient').classList.contains('active'):!!d.managementNewClient,durationMinutes:d.durationMinutes||60}}
    async function saveDraft(){
      capture();
      d.kind=kind;
      if(kind!=='Management'){d.managementNewMember=false;d.managementNewClient=false}
  ...
  function staffReportRows(data){
    ...
    return candidates.map(c=>{
      ...
      const formats=['Group','Semi-Private','1-on-1','Management'].map(kind=>{
      ...
      const managementRows=rows.filter(row=>row.session.kind==='Management');
      const managementNewMemberRows=managementRows.filter(row=>row.session.managementNewMember);
      const managementNewClientRows=managementRows.filter(row=>row.session.managementNewClient);
      const clientSignups=periodClients.filter(client=>client.signupOwnerId===c.id).map(client=>({...client,managementSource:false,source:'Client record'}));
      const managementSignups=managementNewMemberRows.map(row=>({id:`management_member_${row.session.id}`,name:row.session.notes?.trim()||'Management New Member',email:'',phone:'',package:'Management Session',signupOwnerId:c.id,signupDate:row.session.date,managementSource:true,source:'Management session',sessionId:row.session.id,sessionTime:row.session.time}));
      const signups=[...clientSignups,...managementSignups].sort((a,b)=>String(a.signupDate||'').localeCompare(String(b.signupDate||''))||String(a.name||'').localeCompare(String(b.name||'')));
      ...
      return {coach:c,rows,completedRows,completed:completedRows.length,scheduled:rows.length,scheduledHours:round2(rows.reduce((n,r)=>n+(r.session.durationMinutes||60)/60,0)),attendance,formats,signups,managementNewMembers:managementNewMemberRows.length,managementNewClients:managementNewClientRows.length,sessionCommissionRate,signupBonusRate,coachingCommission,memberBonus,totalCommission:round2(coachingCommission+memberBonus)};
    });
  }
  function staffKpis(staff){
    const coaches=staff.filter(r=>r.coach.role!=='Manager');
    const completed=staff.reduce((n,r)=>n+r.completed,0), attendance=staff.reduce((n,r)=>n+r.attendance.attended,0), signups=staff.reduce((n,r)=>n+r.signups.length,0), newClients=staff.reduce((n,r)=>n+(r.managementNewClients||0),0), commission=staff.reduce((n,r)=>n+r.totalCommission,0), booked=staff.reduce((n,r)=>n+r.attendance.booked,0);
    return {completed,attendance,signups,newClients,commission,booked,attendanceRate:booked?Math.round(attendance/booked*100):0,coaches:coaches.length,managers:staff.filter(r=>r.coach.role==='Manager').length};
  }
  ...
  function staffPerformanceCard(r){
    ...
    const secondLabel=manager?'New client markers':'Completed sessions';
    const secondValue=manager?(r.managementNewClients||0):r.completed;
  ...
  function openStaffDashboard(staffId,filters){
    ...
    const manager=r.coach.role==='Manager';
    const signupPanel=`<section class="card"><div class="section-title"><h2>New Member Sign-ups</h2><span>${r.signups.length} attributed</span></div><p class="hint">Member signup incentive is calculated from client records plus Management sessions marked New Member inside the selected range.</p><div class="signup-list">${r.signups.length?r.signups.map(c=>`<div class="signup-row"><div><b>${esc(c.name||'Client')}</b><small>${esc(c.package||'Unassigned')} · ${c.signupDate?prettyDate(c.signupDate):'Signup date not set'}${c.managementSource?' · Management marker':''}</small></div><span>+ ${money(r.signupBonusRate)}</span></div>`).join(''):'<div class="staff-empty">No attributed new members in this range.</div>'}</div></section>`;
  ...
  function addStaffPerformanceSheet(workbook,data,staffId=''){
    ...
    staff.forEach(r=>{const f=Object.fromEntries(r.formats.map(x=>[x.kind,x]));const row=ws.addRow([r.coach.name,r.coach.role,r.completed,f.Group?.completed||0,f['Semi-Private']?.completed||0,f['1-on-1']?.completed||0,f.Management?.sessions||0,r.attendance.attended,r.attendance.rescheduled,r.attendance.cancelled,r.attendance.booked?round2(r.attendance.attended/r.attendance.booked):0,r.sessionCommissionRate,r.coachingCommission,r.signups.length,r.managementNewClients||0,r.signupBonusRate,r.memberBonus,r.totalCommission]);row.getCell(11).numFmt='0%';[12,13,16,17,18].forEach(c=>row.getCell(c).numFmt='$#,##0.00')});
    styleExcelRows(ws,5,cols);if(staff.length){const last=4+staff.length;const total=ws.addRow(['TOTAL','',{formula:`SUM(C5:C${last})`},{formula:`SUM(D5:D${last})`},{formula:`SUM(E5:E${last})`},{formula:`SUM(F5:F${last})`},{formula:`SUM(G5:G${last})`},{formula:`SUM(H5:H${last})`},{formula:`SUM(I5:I${last})`},{formula:`SUM(J5:J${last})`},'', '',{formula:`SUM(M5:M${last})`},{formula:`SUM(N5:N${last})`},{formula:`SUM(O5:O${last})`},'',{formula:`SUM(Q5:Q${last})`},{formula:`SUM(R5:R${last})`}]);total.eachCell(cell=>{cell.font={name:'Aptos',size:10,bold:true,color:{argb:'F8F1E3'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'1F8CFF'}}});[13,17,18].forEach(c=>total.getCell(c).numFmt='$#,##0.00')}return ws;
  }
  ...
  function addMemberSignupSheet(workbook,data,staffId=''){
    const staff=staffReportRows(data).filter(r=>!staffId||r.coach.id===staffId);const rows=staff.flatMap(r=>r.signups.map(c=>({entry:c,staff:r}))).sort((a,b)=>String(a.entry.signupDate||'').localeCompare(String(b.entry.signupDate||''))||String(a.entry.name||'').localeCompare(String(b.entry.name||'')));
    const ws=workbook.addWorksheet('Member Sign-ups',{views:[{state:'frozen',ySplit:4}]});const cols=[{header:'Signup Date',width:16},{header:'Client / Marker',width:24},{header:'Source',width:20},{header:'Email',width:30},{header:'Phone',width:18},{header:'Package',width:24},{header:'Attributed Staff',width:22},{header:'Role',width:18},{header:'Bonus / Member',width:18},{header:'Member Incentive',width:19}];setupExcelSheet(ws,'New Member Sign-ups',`Reporting period: ${reportScopeText(data.filters)} · Includes client records and Management sessions marked New Member`,cols);
    rows.forEach(({entry:c,staff:r})=>{const row=ws.addRow([reportDate(c.signupDate),c.name||'Management New Member',c.managementSource?'Management session':'Client record',c.email||'',c.phone||'',c.package||'Unassigned',r.coach.name,r.coach.role,r.signupBonusRate,r.signupBonusRate]);row.getCell(1).numFmt='mmm d, yyyy';row.getCell(9).numFmt='$#,##0.00';row.getCell(10).numFmt='$#,##0.00'});styleExcelRows(ws,5,cols);if(rows.length){const total=ws.addRow(['','','','','','','','','',{formula:`SUM(J5:J${4+rows.length})`}]);total.eachCell(cell=>{cell.font={name:'Aptos',size:10,bold:true,color:{argb:'F8F1E3'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFD166'}}});total.getCell(10).numFmt='$#,##0.00'}return ws;
  }
